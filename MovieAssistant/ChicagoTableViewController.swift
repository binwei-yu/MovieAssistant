...
class ChicagoTableViewController: UITableViewController {
    ...
    var movies: [Movie] = []
    @IBOutlet var chicagoTableView: UITableView!
    ...
    private func search() {
        movieService.search(for: "Chicago", completion: { movies, error in
            ...
            self.chicagoTableView.reloadData()
        })
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: "ChicagoMovieCell", for: indexPath) as? MovieTableViewCell else {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if let source = sender as? MovieTableViewCell {
            ...
            guard let index = chicagoTableView.indexPath(for: source)?.row else{
